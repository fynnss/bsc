...
func (tree *layerTree) front() common.Hash {
	...
	for _, layer := range tree.layers {
		...
		case *diskLayer:
			if dl.stale {
				log.Info("agg pathdb top disklayer is stale")
				return base
			}
			base = dl.rootHash()
		case *diffLayer:
	...
	if (base == common.Hash{}) {
		log.Info("agg pathdb top difflayer is empty")
		return base
	}
	parent := base
	for {
		...
		if !ok {
			log.Info("agg pathdb top difflayer", "root", parent)
			return parent
		}
		if len(children) != 1 {
			log.Info("agg pathdb top difflayer is forked", "common ancestor root", parent)
			return parent
		}
		parent = children[0]
	}
