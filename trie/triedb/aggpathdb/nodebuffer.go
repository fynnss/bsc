...
// nodebuffer implement trienodebuffer interface, and aysnc the nodecache
// to disk.
...
func (a *nodebuffer) node(owner common.Hash, path []byte, hash common.Hash) (*trienode.Node, error) {
	a.mux.RLock()
	defer a.mux.RUnlock()
	...
	if node == nil {
		node, err = a.background.node(owner, path, hash)
		if err != nil {
			return nil, err
		}
	}
...
// copy.
func (a *nodebuffer) commit(nodes map[common.Hash]map[string]*trienode.Node) *nodebuffer {
	a.mux.Lock()
	defer a.mux.Unlock()
...
func (a *nodebuffer) revert(db ethdb.KeyValueReader, nodes map[common.Hash]map[string]*trienode.Node) error {
	a.mux.Lock()
	defer a.mux.Unlock()
...
// setSize is unsupported in nodebuffer, due to the double buffer, blocking will occur.
func (a *nodebuffer) setSize(size int, db ethdb.KeyValueStore, clean *aggNodeCache, id uint64) error {
	return errors.New("not supported")
}

// reset cleans up the disk cache.
func (a *nodebuffer) reset() {
	a.mux.Lock()
	defer a.mux.Unlock()
...
func (a *nodebuffer) getAllNodes() map[common.Hash]map[string]*trienode.Node {
	a.mux.Lock()
	defer a.mux.Unlock()

	cached, err := a.current.merge(a.background)
	if err != nil {
		log.Crit("[BUG] failed to merge nodecache under revert nodebuffer", "error", err)
	}
	return cached.nodes
}

func (a *nodebuffer) getLayers() uint64 {
	a.mux.RLock()
	defer a.mux.RUnlock()
...
func (a *nodebuffer) getSize() (uint64, uint64) {
	a.mux.RLock()
	defer a.mux.RUnlock()
...
func (nc *nodecache) flush(db ethdb.KeyValueStore, cleans *aggNodeCache, id uint64) error {
	if atomic.LoadUint64(&nc.immutable) != 1 {
	...
	nodes := aggregateAndWriteAggNodes(batch, nc.nodes, cleans)
