...
// nodebuffer is a collection of modified agg aggNodes to aggregate the disk
...
func (b *nodebuffer) canFlush(force bool) bool {
...
func (b *nodebuffer) flush(db ethdb.KeyValueStore, cleans *aggNodeCache, id uint64) error {
	...
	nodes := aggregateAndWriteAggNodes(cleans, batch, b.nodes)
	...
	log.Debug("Persisted aggPathDB aggNodes", "aggNodes", len(b.nodes), "bytes", common.StorageSize(size), "elapsed", common.PrettyDuration(time.Since(start)))
	b.reset()
	return nil
}
...
func aggregateAndWriteAggNodes(cache *aggNodeCache, batch ethdb.Batch, nodes map[common.Hash]map[string]*trienode.Node) (total int) {
